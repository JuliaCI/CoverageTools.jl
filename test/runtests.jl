...
@testset "Processing coverage" begin
    ...
    elseif VERSION.major == 1 && VERSION.minor > 9
        nl = Sys.iswindows() ? "\\r\\n" : "\\n"
        msg = """parsing error in $bustedfile:8: Base.Meta.ParseError(\"ParseError:$(nl)# Error @ none:3:10$(nl)    s = 0$(nl)    for i [1,2,3]   # this line has a parsing error$(nl)#        └─┘ ── invalid iteration spec: expected one of `=` `in` or `∈`\", Base.JuliaSyntax.ParseError(Base.JuliaSyntax.SourceFile(\"function parseerr()$(nl)    s = 0$(nl)    for i [1,2,3]   # this line has a parsing error$(nl)        \", 42, \"none\", 1, [1, 21, 31, 83, 91]), Base.JuliaSyntax.Diagnostic[Base.JuliaSyntax.Diagnostic(82, 84, :error, \"invalid iteration spec: expected one of `=` `in` or `∈`\"), Base.JuliaSyntax.Diagnostic(87, 86, :error, \"invalid iteration spec: expected one of `=` `in` or `∈`\"), Base.JuliaSyntax.Diagnostic(89, 88, :error, \"invalid iteration spec: expected one of `=` `in` or `∈`\"), Base.JuliaSyntax.Diagnostic(89, 89, :error, \"unexpected `]`\"), Base.JuliaSyntax.Diagnostic(89, 88, :error, \"Expected `end`\"), Base.JuliaSyntax.Diagnostic(89, 88, :error, \"Expected `end`\"), Base.JuliaSyntax.Diagnostic(89, 89, :error, \"extra tokens after end of expression\")], :none))"""
    else
